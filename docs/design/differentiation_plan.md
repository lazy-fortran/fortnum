...
| ODE hybridization | forward sensitivity with Enzyme RHS JVP, analytical Cash-Karp discrete adjoint, and stage-level parameter VJP accumulation complete; checkpointing and broader tournaments pending | closed-form trajectory sensitivity, exact matrix-exponential VJP, parameter-count scaling, adjoint identity, and complete-solve finite difference |
