...
| ODE hybridization | forward sensitivity with Enzyme RHS JVP, analytical Cash-Karp discrete adjoint, stage-level parameter VJP accumulation, checkpointed/recomputed reverse, batched analytical implicit-stage tangents/adjoints, transversal event-time JVPs, continuous and discrete sensitivity contracts, and short and long nonstiff tournaments complete; stiff, many-parameter, and event-driven trajectory tournaments remain | closed-form continuous trajectory/event-time sensitivity with refinement, frozen-map finite-difference JVP/VJP, exact matrix-exponential VJP, exact implicit-stage tangent/adjoint, parameter/objective/direction/checkpoint scaling, adjoint identity, and complete-solve finite difference |
