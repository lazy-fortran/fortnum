...
| symbolic generation | one-DAG value, linear-scaling contracted JVP/VJP, fused/separate and proved-equivalent raw/simplified/factored Dawson families, multi-input GPU family, and annotation-only device-leaf emission complete | generated Dawson and multi-input families, symbolic zero proofs, post-CSE ranking, independent formula and adjoint oracles, regeneration gate, real-device tests, scaling benchmarks |
