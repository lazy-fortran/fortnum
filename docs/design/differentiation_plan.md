...
| symbolic generation | one-DAG value, linear-scaling contracted JVP/VJP, fused/separate Dawson family, multi-input GPU family, and annotation-only device-leaf emission complete | generated Dawson and multi-input families, independent formula and adjoint oracles, regeneration gate, real-device tests, scaling benchmarks |
| interpolation interface rules | combined evaluation/value, active support nodes and breakpoints, implicit fitted coefficients, cell/span status, and Lagrange/B-spline interface tournaments complete | finite-difference directional, adjoint, fixed-grid, and scaling evidence |
