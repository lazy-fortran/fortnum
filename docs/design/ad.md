...
|-------------|------------------|-----------|
| `special`   | `analytic_rule` (some `transparent`) | Derivatives follow known recurrences and identities (e.g. `Gamma'/Gamma = psi`); use the closed form where it is more stable, fall back to `transparent` for plain polynomial or rational kernels. |
