...
NEO-2 uses GSL and FGSL as its numerical backends. Replace GSL/FGSL calls with
...
KAMEL uses GSL (special functions, quadrature), libcerf, SLATEC/AMOS (Bessel
via Fortran 77 wrappers), and a custom ODE integrator. Replace the following
where the calling context is smooth:

- GSL special functions: fortnum_special (Bessel, gamma, Dawson)
- GSL quadrature: fortnum_integrate (QAG/QAGS pattern)
