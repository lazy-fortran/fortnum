...
Accelerate the series with terms `terms(1:n)`. Replaces `gsl_sum_levin_u_accel`.
...
(`multiroot_fdf_t`). Replaces `gsl_multiroot_fdfsolver_hybridj`.
...
B-spline basis functions and derivatives over a breakpoint set, in the GSL/FGSL
