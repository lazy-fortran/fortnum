...
| `fortnum_validated_ode` | validated ODE integration with an abstract right-hand side `ode_rhs_t`: `picard_apriori`, `gronwall_bound`, `lohner_step_jacobian`, `lohner_qr_frame`, `lohner_inverse_enclosure`, `taylor_lohner_predictor`, `lohner_state_t`/`lohner_step`/`lohner_integrate`, `event_crossing_newton` |
