...
| libneo / GSL | fortnum primal | derivative product | status |
|---|---|---|---|
| `gsl_sf_bessel_In(n, x)` | `bessel_in(n, x)` | `bessel_in_jvp(n, x, v, jv)` | NOW (`analytic_rule`) |
| `gsl_sf_bessel_Kn(n, x)` | `bessel_kn(n, x)` | `bessel_kn_jvp(n, x, v, jv)` | NOW (`analytic_rule`) |
| `gsl_sf_bessel_In_array` | `bessel_in_array(nmax, x, values)` | no JVP yet | later (recurrence over array; #TBD) |
...
| libneo / GSL | fortnum primal | derivative product | status |
|---|---|---|---|
| `gsl_sf_gamma_inc(a, x)` | `gamma_lower(a, x)` | `gamma_lower_jvp(x, v, jv)` | NOW d/dx (`analytic_rule`) |
| | | d/da | later: requires digamma series (#TBD) |
| `gsl_sf_gamma_inc_P(a, x)` | `gamma_reg_p(a, x)` | | later (depends on d/da of gamma) |
...
|---|---|---|---|
| `odeint` (GSL gsl_odeiv2 or custom) | `ode_integrate(problem, ws, sol, st)` | `ode_integrate_jvp` forward sensitivity | NOW (`trace_rule`) |
...
| libneo / FGSL / QUADPACK | fortnum primal | derivative product | status |
|---|---|---|---|
| `gsl_integration_qag` / `dqag` | `integrate_qag(...)` | `integrate_qag_jvp(dfdp, result, di_dp, st, ctx)` | NOW (`trace_rule`) |
| `gsl_integration_qags` / `dqags` | `integrate_qags(...)` | later (#TBD) | |
| `gsl_integration_qagp` | `integrate_qagp(...)` | later | |
| `gsl_integration_qagiu` | `integrate_qagiu(...)` | later | |
...
| libneo / GSL | fortnum primal | derivative product | status |
|---|---|---|---|
| `gsl_rng_alloc` + `gsl_rng_set(seed)` | `rng_seed(g, seed, status)` | none | never |
| `gsl_rng_uniform(rng)` | `rng_uniform(g, value)` | none | never |
| Standard normal via GSL | `rng_normal(g, value)` | none | never |
| `gsl_rng_free` | nothing (rng_t lives on stack) | | |
