module fortnum_levin
    ...
    !
    ! Replaces the GSL gsl_sum_levin_u_* family used by MEPHIT (hyper1F1.c
    ...
    !
    ! Algorithm (clean-room from the published papers, no GSL/FGSL source):
    !   D. Levin, Int. J. Comput. Math. B3 (1973) 371-388.
    !   E. J. Weniger, Comput. Phys. Rep. 10 (1989) 189-371, eqs 7.2-8, 7.3-9.
    !   Fessler, Ford, Smith, ACM TOMS 9 (1983) 346-354 (GSL's algorithm).
    !
    ...
    !   D_{n-1}^{(0)}.  As in GSL's accel routine the table is grown one term
...
contains
    ...
    subroutine levin_u_accel(terms, n, sum_accel, abserr, status)
        ...
        do i = 1, n
            ...
            ! value, floored by the relative rounding level (GSL accel scheme).
            if (i == 1) then
                est = abs(val)
            else
                est = abs(val - prev)
