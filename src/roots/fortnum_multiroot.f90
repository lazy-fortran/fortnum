module fortnum_multiroot
    ...
    !
    ! Solvers (replace gsl_multiroot_fdfsolver_hybridsj / _hybridsd):
...
contains
    ...
    ! estimate (replaces gsl_deriv_central).
    !
    ...
    ! error estimate, as GSL does.  abserr = |trunc| + |round|.
    subroutine deriv_central(f, x, h, result, abserr, status, ctx)
        ...
        ! Rounding-error scale of each rule (GSL deriv.c form): sum of
        ! |coefficient| * |f| * eps / h.
    ...
    ! Ascending index sort (replaces gsl_heapsort_index): perm is the
