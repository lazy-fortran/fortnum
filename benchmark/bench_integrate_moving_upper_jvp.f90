program bench_integrate_moving_upper_jvp
    ...
    if ((trim(candidate) /= "analytical") .and. &
        ...
        error stop "usage: bench_integrate_moving_upper_jvp " // &
...
contains
    ...
    function run_sample(name, count) result(ns_per_jvp)
        ...
        real(dp) :: bound, derivative, minus_value, plus_value, sink
        ...
        do iteration = 1_int64, count
            ...
            bound = bound_offset + bound_rate*parameter%value
            if (name == "analytical") then
                call integrate_moving_upper_jvp(primal_integrand, &
                    integrand_tangent, 0.0_dp, bound, bound_rate, derivative, &
                    status, epsrel=1.0e-10_dp, ctx=parameter)
            else
                parameter%value = parameter%value + h
                bound = bound_offset + bound_rate*parameter%value
                call integrate(primal_integrand, 0.0_dp, bound, plus_value, &
                    status, epsrel=1.0e-10_dp, ctx=parameter)
                parameter%value = parameter%value - 2.0_dp*h
                bound = bound_offset + bound_rate*parameter%value
                call integrate(primal_integrand, 0.0_dp, bound, minus_value, &
                    status, epsrel=1.0e-10_dp, ctx=parameter)
...
end program bench_integrate_moving_upper_jvp
