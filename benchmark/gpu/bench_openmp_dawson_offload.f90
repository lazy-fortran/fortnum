program bench_openmp_dawson_offload
    ...
    case ("vjp_resident")
        ...
        vjp_workload = .true.
    case default
        error stop "unknown OpenMP benchmark candidate"
    end select
    ...
    if (resident) then
        if (vjp_workload) then
            !$omp target data map(to: x, f, u) map(alloc: adjoints)
            call run_benchmark()
...
contains
    ...
    function run_sample() result(milliseconds)
        ...
        else if (trim(candidate) == "vjp" .or. &
                trim(candidate) == "vjp_resident") then
...
end program bench_openmp_dawson_offload
