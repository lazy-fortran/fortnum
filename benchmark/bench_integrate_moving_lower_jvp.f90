program bench_integrate_moving_lower_jvp
    use, intrinsic :: iso_fortran_env, only: dp => real64, int64
    use fortnum_benchmark_memory, only: peak_rss_bytes
    use fortnum_integrate, only: integrate, integrate_moving_lower_jvp
    ...
    if ((trim(candidate) /= "analytical") .and. &
        ...
        error stop "usage: bench_integrate_moving_lower_jvp " // &
    ...
    function run_sample(name, count) result(ns_per_jvp)
        ...
        do iteration = 1_int64, count
            ...
            if (name == "analytical") then
                call integrate_moving_lower_jvp(primal_integrand, &
                    integrand_tangent, bound_rate*parameter%value, 1.0_dp, &
                    bound_rate, derivative, status, epsrel=1.0e-10_dp, &
                    ctx=parameter)
            else
                parameter%value = parameter%value + h
                call integrate(primal_integrand, bound_rate*parameter%value, &
                    1.0_dp, plus_value, status, epsrel=1.0e-10_dp, &
                    ctx=parameter)
                parameter%value = parameter%value - 2.0_dp*h
                call integrate(primal_integrand, bound_rate*parameter%value, &
                    1.0_dp, minus_value, status, epsrel=1.0e-10_dp, &
                    ctx=parameter)
...
end program bench_integrate_moving_lower_jvp
