program bench_fixed_point_tangent
    ...
    integer(int64), parameter :: reference_reps = 50000_int64
    real(dp), parameter :: h = 1.0e-5_dp
    real(dp) :: p(2), xstar(2), map_x(2, 2), map_p(2, 2), tp(2), warmup
    ...
    if ((trim(candidate) /= "analytical") .and. &
        ...
        error stop "usage: bench_fixed_point_tangent analytical|reference"
    end if
...
contains
    ...
    subroutine report_validation_error()
        ...
        call fixed_point_jvp(map_x, map_p, tp, analytical, status)
        if (.not. status_ok(status)) error stop "fixed-point JVP failed"
        pp = p + h*tp
        pm = p - h*tp
        ...
        if (error > 1.0e-9_dp) error stop "fixed-point JVP validation failed"
        write (*, "(es24.16)") error
    ...
    function run_sample(name, count) result(ns_per_call)
        ...
        real(dp) :: dx(2), pp(2), pm(2), xp(2), xm(2), sink
...
end program bench_fixed_point_tangent
