program bench_scalar_root_adjoint
    use, intrinsic :: iso_fortran_env, only: dp => real64, int64
    use fortnum_benchmark_memory, only: peak_rss_bytes
    use fortnum_roots, only: root_implicit_vjp, root_vjp
    ...
    if ((trim(candidate) /= "assembled") .and. &
        ...
        error stop "usage: bench_scalar_root_adjoint assembled|boundary"
    end if
...
contains
    ...
    function run_sample(name, count) result(ns_per_call)
        ...
        real(dp) :: f_p(2), jtu(2), sink, u

        sink = 0.0_dp
        f_p = [xstar, -1.0_dp]
        call system_clock(tick0, rate)
        do k = 1_int64, count
            u = 0.02_dp*real(mod(k, 17_int64) - 8_int64, dp)
            if (trim(name) == "boundary") then
                call root_implicit_vjp(residual_vjp, xstar, p, u, jtu, status)
            else
                call root_vjp(3.0_dp*xstar*xstar + p(1), f_p, u, &
                    jtu, status)
            end if
            sink = sink + jtu(1)
        end do
    ...
    subroutine residual_vjp(x, parameters, u, f_x, f_p_t_u, context)
        real(dp), intent(in) :: x, parameters(:), u
        real(dp), intent(out) :: f_x, f_p_t_u(size(parameters))
        class(*), intent(inout), optional :: context

        f_x = 3.0_dp*x*x + parameters(1)
        f_p_t_u(1) = x*u
        f_p_t_u(2) = -u
    end subroutine residual_vjp

end program bench_scalar_root_adjoint
